...
const Game4 = () => {
  ...
  const handleSubmit = async (values: { linkb1: string }) => {
    ...
    try {
      ...
      const updateUser = {
        location: 4,
        level: 4, // ✅ Cập nhật trạng thái hoàn thành nhiệm vụ 4
        task: user.task.map((task) => {
          if (task.id === 4) {
            return {
              ...task,
              status: 2, // ✅ Cập nhật trạng thái hoàn thành nhiệm vụ 4
              link: linkb1,
      ...
      login({
        ...user,
        location: 4,
        level: 4, // ✅ Cập nhật trạng thái hoàn thành nhiệm vụ 2
        task: user.task.map((task) => {
          if (task.id === 4) {
            return {
              ...task,
      ...
      navigate("/hoan-thanh-sinh-hoat-4");
    } catch (error) {
